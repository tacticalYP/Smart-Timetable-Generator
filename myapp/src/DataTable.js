...
const DataTable = () => {
  ...
  const {
    array1 = [],
    array2 = [],
    rowData: loadedRowData = null,
    cellTextColors: loadedTextColors = null,
    subjectColors: loadedColors = null,

    storageKey: loadedStorageKey = null
  } = location.state || {};
  ...
  const columnDefs = [
    { headerName: "Time", field: "time", width: 120, editable:true,
      cellStyle: (params) => {
        ...
        const textColor = cellTextColors[`${params.colDef.field}_${params.node.rowIndex}`];
        return {
          backgroundColor: subject ? subjectColors[subject] : {},
          color: textColor || {}
        };
      }
    },
  ...
  const applyTextColor = () => {
    if (selectedCell.rowIndex !== null && selectedCell.colId !== null) {
      const cellKey = `${selectedCell.colId}_${selectedCell.rowIndex}`;
      const newColors = { ...cellTextColors, [cellKey]: textColor };
      ...
      if (storageKey) {
        localStorage.setItem(storageKey, JSON.stringify({
          name: storageKey.replace("timetable_", ""),
          rowData,
          subjectColors,
          cellTextColors: newColors
        }));
      }
    }
  };
  ...
  const applyCellColor = () => {
    ...
    // After changing text/color/cell, update storage
    if (storageKey) {
      localStorage.setItem(storageKey, JSON.stringify({
        name: storageKey.replace("timetable_", ""),
        rowData,
    ...
    rowData.forEach((row) => {
      weekDays.forEach((day) => {
        ...
        if (subject) {
          ...
          let startHour = parseInt(timeRange[0]);
  ...
  useEffect(() => {
    // CASE 2: Loaded from previous timetable
    if (loadedRowData && loadedColors && loadedStorageKey) {
      setRowData(loadedRowData);
      Object.assign(subjectColors, loadedColors);
      if (loadedTextColors) setCellTextColors(loadedTextColors);
      setStorageKey(loadedStorageKey);
      return;
    }
  
    // CASE 1: Fresh timetable using array1 and array2
    subjects = array1;
    slots = array2;
    timeSlots.forEach((time) => {
      subjectColors[time] = `rgb(255, 255, 25)`;
    ...
    const timetableData = generateTimetableData(slotToSubject);
    setRowData(timetableData);
  
    const creationTime = new Date().toLocaleString().replace(/[/, ]/g, "_").replace(/:/g, "-");
    const key = `timetable_${creationTime}`;
    setStorageKey(key);
  ...
  return (
    <div className="table-container">
      ...
      <div className="ag-theme-alpine table-wrapper" ref={tableRef}>
        <AgGridReact ref={gridRef} rowData={rowData} columnDefs={columnDefs} onCellClicked={handleCellClick} onCellValueChanged={textChange}/>
      </div>
